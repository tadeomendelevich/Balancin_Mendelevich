...
|-------|--------------------------|------------------|----------------------|
| 2026-06-27 | Core/Src/main.c | Velocidad en curvas: eliminado floor 25% de `line_desired_forward_vel`. Ahora `desired_vel = LINE_SPEED_TARGET * speed_factor` (0 en curva cerrada). El límite de freno también escala con `speed_factor` (`brake_limit = -1.0 * speed_factor`): en curva cerrada no frena ni acelera, en recta permite hasta -1°. | Robot se aceleraba en curvas porque el floor mantenía desired_vel=0.375 m/s independientemente del error de línea |
...
| 2026-06-27 | Core/Src/main.c | Bug crítico giro en círculos (fix definitivo): `balance_hold_active` ahora excluido en TODO `ROBOT_STATE_LINE_FOLLOWING` (condición simplificada a solo verificar `robot_state`). Esto garantiza `pwm_sat ≠ 0` en todos los estados de línea: el steering residual produce diferencial (giro hacia la línea) en lugar de spin puro. `steering_adjustment` ya NO se limpia cuando `!line_detected` — el último valor conocido ayuda al robot a volver a la línea. Solo se limpia al transicionar a LOST después de 2000ms. Fix anterior (limpiar steering inmediatamente) era incorrecto: hacía que el robot dejara de corregir cuando los sensores salían de la cinta. | Primer fix causaba "robot no sigue la línea": al limpiar steering con !line_detected, el robot no corregía cuando todos los sensores perdían la cinta |
