...
| 2026-06-27 | Core/Src/main.c | Bug crítico giro en círculos: en `LINE_STATE_FOLLOWING` con `line_detected=false` (hasta 2s antes de ir a LOST), `steering_adjustment` conservaba el último valor de corrección. Como `line_detected=false` no está en la lista de exclusión de `balance_hold_active`, el hold podía activarse con `pwm_sat≈0` y steering residual → robot giraba puro en su lugar. Fix: en el bloque `else` (sin línea), se limpian `steering_adjustment=0` y `line_integral=0` inmediatamente, sin esperar los 2000ms de LOST. | Robot seguía bien la línea y de repente giraba en círculos: el balance_hold + steering stale producían spin puro con pwm_sat≈0 |
