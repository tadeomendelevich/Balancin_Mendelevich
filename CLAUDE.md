...
- **`LINE_STATE_OBJ_ARC` implementado**: post-HOLD(2s), el robot avanza en arco con PI cerrado sobre diferencial de encoders hasta re-detectar la línea → vuelve a FOLLOWING. Cadena activa: FOLLOWING → WAIT(2s) → ROTATE → HOLD(2s) → ARC → FOLLOWING. OBJ_REVERSE y OBJ_BRAKE deshabilitados temporalmente.
- PI de arco: `OBJ_ARC_DIFF_TARGET=0.5f` rps (positivo = gira izquierda, convención empírica), `OBJ_ARC_KP=15`, `OBJ_ARC_KI=3`, saturación ±30, anti-windup activo
- `OBJ_DETECT_THRESHOLD_VAL` subido de `2000` → `3200` (sensores hardware actualizados con nuevas resistencias, mayor alcance)
- `rev_target_counts` reducido de `300` → `150` counts (la mitad del recorrido de reversa anterior)
