...
- **Avance de línea subordinado a estabilidad:** el seguidor de línea solo modula el setpoint de inclinación. Si `roll_filtrado_grados` se aleja del equilibrio o `giro_filtrado` sube, `escala_estabilidad` reduce el avance pedido. El PWM final sigue bajo autoridad del PID de balance.
...
- **Escape de reversa por encoders:** si `velocidad_est_f` es positiva (convención actual: reversa), `linea_escape_reversa_angulo` suma inclinación hacia adelante de forma rampeada. Es más fuerte que el boost de stall, pero sigue actuando solo sobre setpoint.
