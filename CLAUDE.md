...
| 2026-07-27 | Core/Src/main.c | **11ª pasada — factor de CALMA global: la estación se apaga entera cuando el cuerpo está quieto.** Tras la 10ª el reporte siguió igual ("no entra nunca en la siesta, muy brusco en el punto dulce, quiero bajar la respuesta ahí"). Patrón de las pasadas 8-10: suavizar término por término (rodilla de la D, tope del P, drenaje del disp) no alcanzaba — siempre quedaba algún término activo perturbando el setpoint en el punto dulce. Fix estructural en `WheelStation_AngleCorr()`: **`calm = min(1, max(\|gyro_f\|/WHEEL_CALM_GYRO_REF, \|v_trans\|/WHEEL_CALM_V_REF))`** (refs: 8°/s y 1.0 rps) multiplica TODA la corrección (P y D juntos). Cuerpo quieto → calm→0 → estación MUDA: cero perturbación de setpoint, el PID de balance + hold + siesta son los dueños del punto dulce. Empujón → el gyro dispara al instante (sin la lag de los encoders) → estación entera sin retraso. La rodilla `WHEEL_V_SOFT_KNEE` quedó reemplazada (el término \|v\|/V_REF del calm hace lo mismo y más); tope del P (0.5°) y drenaje rápido (150ms) se mantienen. Knobs: sigue brusco en calma → subir REFs; no amortigua llegadas suaves → bajar GYRO_REF a 6. NO compilado (el usuario compila). | Usuario: "no entra nunca en la ventana de siesta, está siendo muy brusco en el punto dulce — quiero bajar de alguna forma la respuesta del robot para que sea más suave ahí" |
| 2026-07-27 | Core/Src/main.c | **10ª pasada — drenaje rápido de la memoria de desplazamiento en reposo real: el residuo del P mantenía al robot gateando y la siesta nunca entraba.** La 9ª dejó el punto dulce suave (VALIDADO) pero la siesta seguía sin entrar jamás. Causa: aun con el tope de 0.5°, el residuo del P se drena con la fuga de τ≈5s — durante esos segundos el setpoint queda corrido y el robot GATEA de vuelta despacito; cada tick del gateo resetea `enc_quiet` (60ms) + los 80ms sostenidos → la ventana de la siesta no se cumple nunca. La ventana en sí está bien (era alcanzable antes de la estación, "cuando entra queda muy eficiente"): el bloqueo era el gateo inducido. Fix en `WheelStation_AngleCorr()`: **reposo real** (ambas ruedas >`WHEEL_DISP_CALM_MS=150`ms sin un tick + \|v_trans\| bajo la zona muerta) → `wheel_disp_f *= WHEEL_DISP_FAST_LEAK=0.90` (τ≈100ms): el residuo muere en ~0.3s de quietud, el setpoint vuelve a 0, el robot deja de gatear y la siesta entra. Coherente con la filosofía sin ancla (quieto de verdad = no hay adonde volver). La deriva lenta REAL (~cm/s cuesta abajo) tiquea cada <150ms y NO dispara el drenaje — el P anti-deriva sigue vivo ahí. NO compilado (el usuario compila). | Usuario: "ahora quedó suave, pero nunca entra en modo siesta de motores — está muy complicado de ingresar o la ventana está muy alta" |
| 2026-07-27 | Core/Src/main.c | **9ª pasada — el brusco del punto dulce era el término P saturado, no la D: tope propio `WHEEL_STATION_P_MAX=0.5°`.** La rodilla de la 8ª no alcanzó ("sigue demasiado brusco y potente en el punto dulce, no entra nunca a la siesta") porque atacaba el término equivocado: a v→0 la D ya se apaga sola — el que quedaba empujando era el **P del desplazamiento reciente**. Números: un empujón que desplaza ~30cm mete ~90 counts en `wheel_disp_f` y con la fuga alargada a τ≈5s (6ª pasada) eso vale 90×0.05=4.5° → saturaba el tope general de 2.5°: al frenar en el punto dulce el robot recibía 2.5° de inclinación de vuelta durante segundos — se pasaba del equilibrio y el error nunca bajaba de los 0.8° de entrada de la siesta. Fix: el P tiene ahora tope PROPIO de **0.5°** (`p_term` clampeado antes de sumar; la D conserva la autoridad grande de ±2.5° que es la validada contra empujones). 0.5° alcanza de sobra para el rol real del P (frenar deriva lenta de mm/s) y queda DEBAJO del umbral de la siesta → tras un empujón el residuo del P ya no bloquea la entrada a dormir. Rodilla de la 8ª se mantiene (suaviza la D en la llegada). NO compilado (el usuario compila). | Usuario: "sigue quedando demasiado brusco y potente en el punto dulce, no logra entrar en siesta los motores nunca" |
| 2026-07-27 | Core/Src/main.c | **8ª pasada — rodilla suave de la D contra la brusquedad en el punto dulce.** La 7ª quedó **VALIDADA** ("la mejora es una locura, amortigua los empujones muy rápido" — commit+push en `6a286a6`), pero al llegar al punto dulce era demasiado brusco: con la D ya en fase y ganancia completa a cualquier velocidad, una velocidad chiquita de llegada (o el pico de un count suelto) metía un golpe de setpoint que lo pasaba del equilibrio justo cuando estaba por entrar a la siesta. Fix: `WHEEL_V_SOFT_KNEE=1.0` rps en `WheelStation_AngleCorr()` — debajo de la rodilla la ganancia de la D escala lineal con \|v\| (`v_soft = min(1, \|v_act\|/rodilla)`, respuesta cuadrática en chico, entera en grande): la llegada es proporcionalmente más suave y no se pasa de largo, pero un empujón real (v > 1 rps) recibe la D completa que ya gustó. NO es zona muerta (la lección de `WHEEL_V_DB_RPS=0.8`): sigue corrigiendo en fase a toda velocidad, solo más blando en chico. Knobs: vaivén chico que no muere → rodilla 1.0→0.6; sigue brusco al asentarse → 1.0→1.5. NO compilado (el usuario compila). | Usuario: "quedó muchísimo mejor, la mejora es una locura — pero es demasiado brusco cuando está por quedarse en el punto dulce y poner siesta: se pasa del equilibrio; me gustó que amortigua los empujones muy rápido" |
