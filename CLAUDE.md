...
│   │   ├── control_motores.c         ← salida PWM TIM3/TIM4, signos, canales y limites
│   │   ├── boton_usuario.c          ← gestos KEY sin decisiones de modo
│   │   ├── telemetria.c             ← CSV USB, log WiFi y odometria periodica
│   │   ├── comunicacion_usb.c       ← RX hacia UNER, cola TX USB CDC y depuracion
...
│       ├── i2c_manager.h
│       ├── control_motores.h         ← API de inicializacion y potencia firmada
│       ├── boton_usuario.h          ← eventos de pulsacion, largo y multiclick
│       ├── telemetria.h             ← muestra de solo lectura del control
│       ├── comunicacion_usb.h       ← API comun de transporte y debug USB
...
- **Última sesión:** 2026-09-21 (display, USB, boton, telemetria y salida PWM de motores separados; Debug/Release y tests host verificados; pendiente prueba fisica)
...
|-------|--------------------------|------------------|----------------------|
| 2026-09-21 | Core/Src/main.c, CLAUDE.md, CHANGELOG.md | Eliminadas nueve variables sin lectores ni direcciones compartidas: es250us, es2ms, contador_alive, contador_mpu6050, mpu_dato_listo, vel_desde_accel, manual_setpoint_rampeado, linea_angulo_rampeado y linea_direccion_fb_int. Todos sus usos eran declaraciones o asignaciones simples de constantes/valores ya calculados, sin llamadas ni accesos a perifericos. Busqueda en Core, USB_DEVICE, Drivers, Middlewares y tests, incluidos UNER y callbacks. Verificado automaticamente que el diff de main contiene exclusivamente esas eliminaciones. main: 6313 a 6291 lineas (-22). Debug/Release compilan sin errores ni advertencias; 43/46 secciones de codigo del objeto Release identicas al respaldo (no se afirma identidad binaria completa). | Respaldo previo de motores: ac89e57 publicado, tag backup/pre-codigo-muerto-2026-09-21. Limpieza conservadora a pedido del usuario. Qt no requiere cambios. Sin prueba fisica en esta sesion. |
| 2026-09-21 | Core/Src/main.c, control_motores.c/.h, Debug/Core/Src/subdir.mk, Debug/objects.list, tests/test_control_motores.c, tests/motores_stubs | Capa de hardware de motores separada: inicializacion PWM, limite -100..100, conversion porcentaje a CCR y seleccion de canales TIM3/TIM4. `Ctrl_SalidaMotores` conserva sin cambios las decisiones de PID, zona muerta, compensacion estatica/cinetica y siesta. Se mantuvieron los signos de las llamadas, el cableado invertido del motor izquierdo, el recorte de 100% a ARR y la parada de los cuatro canales sin reescribir CCR. main: 6390 a 6313 lineas. Debug/Release sin errores ni advertencias; prueba nativa sobre el modulo real con las 90601 combinaciones de -150..150, extremos int16, arranque y parada. | Respaldo previo publicado: 5463f35, tag backup/pre-motores-2026-09-21. Sin cambios requeridos en Qt. Pendiente prueba fisica de ambos sentidos y frenado. |
| 2026-09-21 | Core/Src/main.c, boton_usuario.c/.h, telemetria.c/.h, Debug/Core/Src/subdir.mk, Debug/objects.list, tests/test_boton_regression.py, tests/test_telemetria.c | Boton separado como detector de eventos: main conserva las acciones de modo/alertas; mismas ventanas estrictas 20/400/800 ms y arranque SoftAP sin click. Telemetria concentra cadencias, encabezado CSV, armado WiFi/odometria y conversiones; muestra estatica de solo lectura desde control. Corregida lectura fuera del buffer CSV: 128 a 320 bytes persistentes y envio solo si snprintf cabe completo. Columnas, escalas y paquetes intactos. main: 6524 a 6390 lineas. Debug/Release sin errores ni advertencias; 60450 muestras de boton equivalentes al respaldo, pruebas de CSV/binarios, flags, frecuencias y overflow de tick/secuencia. | Respaldo previo publicado: 867d14b, tag backup/pre-boton-telemetria-2026-09-21. Sin cambios requeridos en Qt. Pendiente prueba fisica. |
| 2026-09-21 | Core/Src/comunicacion_usb.c, Core/Inc/comunicacion_usb.h, main.c, UNER.c, ESP01.c/.h, display_oled.c, Debug/Core/Src/subdir.mk, Debug/objects.list, tests/test_comunicacion_usb.c, tests/usb_stubs | Transporte USB extraido de main: RX hacia UNER, cola circular TX de 512 bytes, paquetes de 64 bytes y mini-printf de debug. Cuerpos de funciones conservados; registro del callback en el mismo punto del arranque. Prototipos centralizados en comunicacion_usb.h. main: 6722 a 6524 lineas. Debug/Release sin errores ni advertencias; pruebas host de RX, wrap, cola llena, atomismo de segmentos, IRQ, lifetime asincrono, reintentos BUSY/FAIL y formato debug; regresiones ESP01/UNER aprobadas. | Respaldo del display publicado antes de modificar: 12904bb, tag backup/pre-usb-2026-09-21. Protocolo, telemetria, control y codigo generado por CubeMX sin cambios de comportamiento. Sin cambios necesarios en Qt; pendiente prueba fisica USB. |
...
- **Hardware de motores aislado (2026-09-21):** `control_motores.c` solo traduce dos porcentajes firmados a los canales PWM TIM3/TIM4. La selección de potencia, PID, compensación de zona muerta y siesta permanecen en `Ctrl_SalidaMotores`, porque dependen del estado del robot y de los encoders. Este límite deja el mapeo eléctrico en un módulo pequeño sin trasladar ni duplicar la política de control.
- **Boton y telemetria aislados (2026-09-21):** `boton_usuario.c` solo reconoce gestos; la politica de modos y alertas sigue en main. `telemetria.c` posee temporizadores, secuencia y formatos; recibe una muestra estatica por ciclo sin acceso de escritura al control. CSV admite filas largas completas sin leer fuera del buffer.
- **Transporte USB aislado (2026-09-21):** `comunicacion_usb.c` conserva la cola y el buffer persistente requerido por CDC. UNER mantiene el protocolo compartido USB/WiFi; desde la extraccion siguiente, `telemetria.c` prepara los mensajes a partir de una muestra del controlador. Los consumidores incluyen una sola API en vez de repetir prototipos `extern`.
