...
- **Freeze residual al mover el robot** — Mitigado con IWDG (watchdog 2s, ver registro 2026-05-09). Si el loop principal se congela por cualquier motivo (I2C lockup, interrupt storm, HardFault, etc.) el MCU resetea solo. El robot pierde el balance ~1s (tiempo de boot) y se recupera sin intervención manual. Si el freeze sigue siendo frecuente, la causa raíz más probable es un I2C lockup por wires flojos del MPU/SSD1306 que generan glitch al mover el robot.
...
- PI de velocidad del seguidor de línea posiblemente en retroalimentación positiva: `vel_enc = -((dr+dl)/2)*scale` → cuando el robot avanza, `velocity_est_f` puede ser negativo. Si es así, `vel_error = desired_vel - (negativo)` crece en vez de decrecer → el PI siempre satura en `+LINE_ANGLE`. El robot igual avanza (setpoint positivo = avance) pero sin regulación de velocidad. Verificar signo en telemetría Qt durante MOVE_FORWARD: si `velocity_est_f < 0`, corregir `desired_vel = -LINE_SPEED_TARGET * speed_factor`
...
| 2026-05-09 | Core/Src/main.c | Límite dinámico de pwm_sat en curvas: `turn_pwm_limit = max(8, 40*(1-|line_error|*0.8))`. Recto: límite=40. Error=0.5: límite=24. Error=1 (borde): límite=8. Efecto inmediato sin esperar rampa del setpoint. | Aceleración excesiva en curvas del seguidor de línea |
