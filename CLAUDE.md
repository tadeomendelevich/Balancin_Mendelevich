...
| Integral anti-windup | `INTEGRAL_MAX` | ±100.0 PWM units; decay=0.990 por ciclo |
| Freno por velocidad | `KV_FRENO` / `KV_FRENO_FUERTE` | 0.0 / 5.0 (umbral FRENO_VEL_UMBRAL=1.5) — velocidad calculada por encoders |
...
- Freno dinámico por velocidad de encoders (KV_FRENO_FUERTE=5.0, umbral 1.5 m/s)
...
- **Avance de línea subordinado a estabilidad:** el seguidor de línea solo modula el setpoint de inclinación. Si `roll_filtrado_grados` se aleja del equilibrio o `giro_filtrado` sube, `stability_scale` reduce el avance pedido. El PWM final sigue bajo autoridad del PID de balance.
...
- **Escape de reversa por encoders:** si `velocidad_est_f` es positiva (convención actual: reversa), `linea_reversa_empuje` suma inclinación hacia adelante de forma rampeada. Es más fuerte que el boost de stall, pero sigue actuando solo sobre setpoint.
